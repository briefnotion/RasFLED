...
// *
// *    Department : (R+D)^2                        Name: timedeventsystem.cpp
  ...
  for(int group=0; group < sdSysData.CONFIG.LED_MAIN.at(0).g_size(); group++)
  {
    for(int strip=0; strip < sdSysData.CONFIG.LED_MAIN.at(0).s_size(group); strip++)
    {
      ...
      if (teEvent[channel].teDATA.size() > 0)
      {
        ...
        {
          if (tmeCurrentTime >= teEvent[channel].teDATA[event].tmeSTARTTIME && 
                teEvent[channel].teDATA[event].booCOMPLETE == false )
          {
            ...
            switch (teEvent[channel].teDATA[event].bytANIMATION)
            {
              ...
              {  
                ...
                // Activate an Animation Set
                {
                  ...
                  // Color Specific Channel Pulse
                  {
                    vdChannelLightFlashColor00(cons, sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip), teEvent, tmeCurrentTime, teEvent[channel].teDATA[event].crgbCOLORSTART1);
                    break;
                  }
                  ...
                  // Color Specific Channel Pulse
                  {
                    vdChannelLightPulseSimple01(cons, sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip), teEvent, tmeCurrentTime, teEvent[channel].teDATA[event].crgbCOLORSTART1);
                    break;
                  }
                  ...
                  // Color Specific Channel Pulse
                  {
                    vdChannelLightPulseColor01(cons, sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip), teEvent, tmeCurrentTime, teEvent[channel].teDATA[event].crgbCOLORSTART1);
                    break;
                  }
                  ...
                  // Color Specific Channel Pulse
                  {
                    ...
                    if (sdSysData.cdTIMER.is_triggered() == true)
                    {
                      if(sdSysData.cdTIMER.is_checked() == false)
                      {
                        ...
                        vdChannelLightFlashColor00(cons, sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip), teEvent, tmeCurrentTime, CRGB(96,96,96));
                      }
                    }
                    break;
                  }
                  ...
                  {
                    if(sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip).strNAME == "Overhead")
                    {
                      vdOverheadIllum(cons, sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip), teEvent, tmeCurrentTime, teEvent[channel].teDATA[event].crgbCOLORSTART1);
                    }
                    break;
                  }
                  ...
                  // Color Specific Illumination
                  {
                    vdHazard(cons, sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip), teEvent, tmeCurrentTime);
                    break;
                  }
                  ...
                  // Color Specific Channel Pulse
                  {
                    if(sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip).strNAME == "Overhead")
                    {
                      vdPacificaishAnimationADVColor(cons, sdSysData.CONFIG.LED_MAIN.at(0).vLED_GROUPS.at(group).vLED_STRIPS.at(strip), teEvent, tmeCurrentTime, teEvent[channel].teDATA[event].crgbCOLORSTART1);
                    }
                    break;
                  }                
